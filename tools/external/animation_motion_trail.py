...
def get_curves(obj, child=False):
	...
	elif obj.animation_data and obj.animation_data.use_nla:
		curves, strips = [], []
...
def screen_to_world(context, x, y):
	depth_vector = view3d_utils.region_2d_to_vector_3d(context.region, context.region_data, [x, y])
	vector = view3d_utils.region_2d_to_location_3d(context.region, context.region_data, [x, y],depth_vector)
	return vector


# turn 3d world coordinates vector into screen coordinate integers (x,y)
def world_to_screen(ctx, vector):
	coord = bpy_extras.view3d_utils.location_3d_to_region_2d(bpy.context.region, bpy.context.space_data.region_3d,vector)
	return coord
...
def get_location(frame, display_ob, offset_ob, curves):
	if offset_ob:
		...
		if offset_ob:
			loc = display_mat.to_translation() + offset_ob.matrix_world.to_translation()
		else:
...
def get_original_animation_data(ctx, keyframes):
	keyframes_ori = {}
	handles_ori = {}

	if ctx.active_object and ctx.active_object.mode == 'POSE':
		armature_ob = ctx.active_object
		objects = [[armature_ob, pb, armature_ob] for pb in
					ctx.selected_pose_bones]
	else:
		objects = [[ob, False, False] for ob in ctx.selected_objects]
	...
	for action_ob, child, offset_ob in objects:
		...
		frame_old = ctx.scene.frame_current
		...
		if ctx.scene.frame_current != frame_old:
			ctx.scene.frame_set(frame_old)

	return(keyframes_ori, handles_ori)


# callback function that calculates positions of all things that need be drawn
def calc_callback(self, ctx):
	motion_trail = ctx.window_manager.motion_trail

	if ctx.active_object and ctx.active_object.mode == 'POSE':
		armature_ob = ctx.active_object
		objects = [[armature_ob, pb, armature_ob] for pb in ctx.selected_pose_bones]
	else:
		objects = [[ob, False, False] for ob in ctx.selected_objects]

	selection_change = objects != self.displayed

	if self.lock and not selection_change and ctx.region_data.perspective_matrix == self.perspective and not motion_trail.force_update:
		return
	...
	if selection_change:
		...
		self.edit_bones = {}
	if selection_change or not self.lock or motion_trail.force_update:
		# contains locations of path, keyframes and timebeads
		self.cached = {"path": {}, "keyframes": {}, "timebeads_timing": {},	"timebeads_speed": {}}
	...
	self.perspective = ctx.region_data.perspective_matrix.copy()
	self.displayed = objects  # store, so it can be checked next time
	motion_trail.force_update = False
	
	try:
		
		global_undo = ctx.preferences.edit.use_global_undo
		...
		for action_ob, child, offset_ob in objects:
			...
			if motion_trail.path_before == 0:
				range_min = ctx.scene.frame_start
			else:
				range_min = max(ctx.scene.frame_start, ctx.scene.frame_current - motion_trail.path_before )
			if motion_trail.path_after == 0:
				range_max = ctx.scene.frame_end
			else:
				range_max = min(ctx.scene.frame_end, ctx.scene.frame_current + motion_trail.path_after )
			# fcx, fcy, fcz = curves
			if child:
				display_ob = child
			else:
			...
			speeds = []
			frame_old = ctx.scene.frame_current
			step = 11 - motion_trail.path_resolution
			...
			for frame in range(range_min, range_max + 1, step):
				...
				if not ctx.region or not ctx.space_data:
					continue
				
				x, y = world_to_screen(ctx, loc)
				
				if motion_trail.path_style == 'simple':
			...
			if motion_trail.path_style == 'speed':
				speeds.sort()
				min_speed = speeds[0]
			...
			elif motion_trail.path_style == 'acceleration':
				accelerations = []
				prev_speed = 0.0
				for i, [x, y, d_loc, frame, action_ob, child] in enumerate(path):
			...
			for fc in curves:
				for kf in fc.keyframe_points:
					...
					if motion_trail.mode == 'location':
						if kf.co[0] not in handle_difs:
							handle_difs[kf.co[0]] = {"left": mathutils.Vector((0,0,0)),
								"right": mathutils.Vector((0,0,0)), "keyframe_loc": None}
					...
					if use_cache and co in self.cached["keyframes"][display_ob.name]:
			...
			if motion_trail.mode == 'location' and motion_trail.handle_display:
				...
				for frame, vecs in handle_difs.items():
					...
					if vecs["keyframe_loc"] is not None:
						...
						vec_keyframe = get_location(frame, display_ob, offset_ob, curves)
					x_left, y_left = world_to_screen(ctx, vec_left * 2 + vec_keyframe)
					x_right, y_right = world_to_screen(ctx, vec_right * 2 + vec_keyframe)
					handles[frame] = {"left": [x_left, y_left],	"right": [x_right, y_right]}
					click.append([frame, "handle_left", mathutils.Vector([x_left, y_left]), action_ob, child])
					click.append([frame, "handle_right", mathutils.Vector([x_right, y_right]), action_ob, child])
				self.handles[display_ob.name] = handles

			# calculate timebeads for timing mode
			if motion_trail.mode == 'timing':
				timebeads = {}
				n = motion_trail.timebeads * (len(kf_time) - 1)
				...
				for i in range(1, n + 1):
					...
					if use_cache and frame in self.cached["timebeads_timing"][display_ob.name]:
					...
					x, y = world_to_screen(ctx, loc)
					timebeads[frame] = [x, y]
					click.append([frame, "timebead", mathutils.Vector([x, y]), action_ob, child])
				self.timebeads[display_ob.name] = timebeads

			# calculate timebeads for speed mode
			if motion_trail.mode == 'speed':
				angles = dict([[kf, {"left": [], "right": []}] for kf in self.keyframes[display_ob.name]])
				for fc in curves:
				...
				for frame, sides in angles.items():
					if sides["left"]:
						...
						if use_cache and bead_frame in self.cached["timebeads_speed"][display_ob.name]:
						...
						click.append( [bead_frame, "timebead", mathutils.Vector([x, y]), action_ob, child] )
					if sides["right"]:
						...
						if use_cache and bead_frame in self.cached["timebeads_speed"][display_ob.name]:
							...
							loc = get_location(bead_frame, display_ob, offset_ob, curves)
							self.cached["timebeads_speed"][display_ob.name][bead_frame] = loc
						x, y = world_to_screen(ctx, loc)
						timebeads[bead_frame] = [x, y]
						click.append( [bead_frame, "timebead", mathutils.Vector([x, y]), action_ob, child] )
			...
			if motion_trail.frame_display:
				path = self.paths[display_ob.name]
				for x, y, color, frame, action_ob, child in path:
					click.append([frame, "frame", mathutils.Vector([x, y]), action_ob, child])
		...
		ctx.preferences.edit.use_global_undo = global_undo
...
def draw_callback(self, context):
	# polling
	if (context.mode not in ('OBJECT', 'POSE') or not context.window_manager.motion_trail.enabled):
		return
	...
	if context.window_manager.motion_trail.path_style == 'simple':
		self.shader.uniform_float("color",(0,0,0,alpha))
		...
		for objectname, path in self.paths.items():
			for i, [x, y, color, frame, action_ob, child] in enumerate(path):
				...
				if i != 0:
					...
					self.shader.uniform_float("color",(r, g, b, alpha))
...
def drag(ctx, event, drag_mouse_ori, active_keyframe, active_handle, active_timebead, keyframes_ori, handles_ori, edit_bones):
	motion_trail = ctx.window_manager.motion_trail
	# change 3d-location of keyframe
	if motion_trail.mode == 'location' and active_keyframe:
		objectname, frame, frame_ori, action_ob, child = active_keyframe

		mat = action_ob.matrix_world.copy().inverted() * edit_bones[child.name].copy().to_4x4() if child else 1

		mouse_ori_world = screen_to_world(ctx, drag_mouse_ori[0], drag_mouse_ori[1]) * mat
		vector = screen_to_world(ctx, event.mouse_region_x, event.mouse_region_y) * mat
	...
	elif motion_trail.mode == 'location' and active_handle:
			...
			for kf in curve.keyframe_points:
				if kf.co[0] == frame:
					if side == "left":
						...
						if kf.handle_left_type in ('AUTO', 'AUTO_CLAMPED', 'ANIM_CLAMPED'):
							kf.handle_left_type = 'ALIGNED'
						elif kf.handle_left_type == 'VECTOR':
							kf.handle_left_type = 'FREE'
						# change handle position(s)
						kf.handle_left[1] = handles_ori[objectname][frame]["left"][i][1] + d[i]
						if kf.handle_left_type in ('ALIGNED', 'ANIM_CLAMPED', 'AUTO', 'AUTO_CLAMPED'):
						...
						# change handle type, if necessary
						if kf.handle_right_type in ('AUTO', 'AUTO_CLAMPED', 'ANIM_CLAMPED'):
							kf.handle_left_type = 'ALIGNED'
						...
						# change handle position(s)
						kf.handle_right[1] = handles_ori[objectname][frame]["right"][i][1] + d[i]
						if kf.handle_right_type in ('ALIGNED', 'ANIM_CLAMPED', 'AUTO', 'AUTO_CLAMPED'):
	...
	# change position of all keyframes on timeline
	elif motion_trail.mode == 'timing' and active_timebead:
		...
		dx_screen = dx_screen / ctx.region.width * range
	...
	# change position of active keyframe on the timeline
	elif motion_trail.mode == 'timing' and active_keyframe:
		objectname, frame, frame_ori, action_ob, child = active_keyframe

		matrix_world = action_ob.matrix_world
		mat = matrix_world.copy().inverted() * edit_bones[child.name].copy().to_4x4() if child else matrix_world.copy().inverted()
		...
		if d_screen.length != 0:
			d_screen = d_screen.length / (abs(d_screen[0]) / d_screen.length *
					  ctx.region.width + abs(d_screen[1]) / d_screen.length *
					  ctx.region.height)
	...
	# change position of active timebead on the timeline, thus altering speed
	elif motion_trail.mode == 'speed' and active_timebead:
		objectname, frame, frame_ori, action_ob, child = active_timebead
		mat = action_ob.matrix_world.copy().inverted() * edit_bones[child.name].copy().to_4x4() if child else 1
		...
		vec_prev = (mathutils.Vector(keyframes_ori[objectname][kf_prev][1]) * mat - loc_ori).normalized()
		vec_next = (mathutils.Vector(keyframes_ori[objectname][kf_next][1]) * mat - loc_ori).normalized()
		d_normal = d.copy().normalized()
		dist_to_next = (d_normal - vec_next).length
		dist_to_prev = (d_normal - vec_prev).length
		
		direction = 1 if dist_to_prev < dist_to_next else -1
...
def cancel_drag(ctx, active_keyframe, active_handle, active_timebead, keyframes_ori, handles_ori, edit_bones):
	motion_trail = ctx.window_manager.motion_trail
	...
	if motion_trail.mode == 'location' and active_keyframe:
		...
		if child:
			loc_ori = loc_ori * edit_bones[child.name] * active_ob.matrix_world.copy().inverted()
		for i, curve in enumerate(curves):
			for kf in curve.keyframe_points:
				if kf.co[0] == frame:
					kf.co[1] = loc_ori[i]
...
def get_handle_type(active_keyframe, active_handle):
	motion_trail = bpy.context.window_manager.motion_trail
	if active_keyframe:
	...
	motion_trail.handle_type_frame = frame
	motion_trail.handle_type_side = side
	motion_trail.handle_type_action_ob = action_ob.name

	motion_trail.handle_type_child = child.name if child else ""
...
def insert_keyframe(self, ctx, frame):
	...
	ctx.window_manager.motion_trail.force_update = True
	calc_callback(self, ctx)


# change the handle type of the active selection
def set_handle_type(self, context):
	motion_trail = context.window_manager.motion_trail
	if not motion_trail.handle_type_enabled:
		return
	if motion_trail.handle_type_old == motion_trail.handle_type:
		# function called because of selection change, not change in type
		return
	motion_trail.handle_type_old = motion_trail.handle_type

	frame = motion_trail.handle_type_frame
	side = motion_trail.handle_type_side
	action_ob = motion_trail.handle_type_action_ob
	action_ob = bpy.data.objects[action_ob]
	child = motion_trail.handle_type_child
	if child:
		child = action_ob.pose.bones[child]
	new_type = motion_trail.handle_type
	...
	for c in curves:
		for kf in c.keyframe_points:
			if kf.co[0] == frame:
				# align if necessary
				if side in ("right", "both") and new_type in ("AUTO", "AUTO_CLAMPED", "ALIGNED"):
					# change right handle
				...
				elif side == "left" and new_type in ("AUTO", "AUTO_CLAMPED", "ALIGNED"):
					# change left handle
	...
	motion_trail.force_update = True


class Anim_OT_MotionTrail(Operator):
	...
	@staticmethod
	def handle_add(self, ctx):
		SpaceView3D = bpy.types.SpaceView3D
		Anim_OT_MotionTrail._handle_calc = SpaceView3D.draw_handler_add(calc_callback, (self, ctx), 'WINDOW', 'POST_VIEW')
		Anim_OT_MotionTrail._handle_draw = SpaceView3D.draw_handler_add(draw_callback, (self, ctx), 'WINDOW', 'POST_PIXEL')

	@staticmethod
	def handle_remove():
		if Anim_OT_MotionTrail._handle_calc is not None:
			bpy.types.SpaceView3D.draw_handler_remove(Anim_OT_MotionTrail._handle_calc, 'WINDOW')
		if Anim_OT_MotionTrail._handle_draw is not None:
			bpy.types.SpaceView3D.draw_handler_remove(Anim_OT_MotionTrail._handle_draw, 'WINDOW')
		Anim_OT_MotionTrail._handle_calc = None
		Anim_OT_MotionTrail._handle_draw = None
	...
	def invoke(self, context, event):
		...
		if not context.window_manager.motion_trail.enabled:
			...
			Anim_OT_MotionTrail.handle_add(self, context)
...
class PANEL_PT_MotionTrail(Panel):
...
class MotionTrailProps(PropertyGroup):
