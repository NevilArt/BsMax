...
def is_free_node_input(input):
	return len(input.links) == 0


def get_light_field(col, light):
	data = light.data
	row = col.row()
...
def get_camera_field(row, camera, active):
	...
	srow.operator(
		'object.select_by_name', text=camera.name
	).name = camera.name
		...
		for node in material.node_tree.nodes:
			if node.type == 'OUTPUT_MATERIAL':
				...
				for link in node.inputs['Surface'].links:
					...
					# TODO fix this issue for blender
	...
	def draw(self, ctx):
		...
		for light in self.lights:
			if light == ctx.active_object:
				get_active_light_field(col, light)
				break
