...
def key_map_key(self):
	if self._key:
		return self._key

	if not self.space in self.keymaps:
		return None
...
def key_maps_new(
		self, space, idname, type, value, properties,
		alt, ctrl, shift, any, modal, repeat, direction):
...
def key_maps_mute(self, space, idname, inputtype, value,
	alt, ctrl, shift,any, modal, direction):
...
def key_maps_set_mute(self, state):
...
def key_maps_register(self):
	self.unregister()
	for k in self.newkeys:
		if k.modal:
			#TODO check for how to change modal key maps too
			keymapitem = k.space.keymap_items.new_modal(
				k.idname, k.type, k.value,
				alt=k.alt, ctrl=k.ctrl, shift=k.shift, any=k.any
			)
		else:
			keymapitem = k.space.keymap_items.new(
				k.idname, k.type, k.value,
				alt=k.alt, ctrl=k.ctrl, shift=k.shift,
				any=k.any, repeat=k.repeat
			)
...
class KeyMap:
	...
	@property
	def key(self):
		return key_map_key(self)
	
	def compare(self, space, idname, type, value,
			alt, ctrl, shift, any, direction, properties):
		return key_map_compare(self, space, idname, type, value,
				alt, ctrl, shift, any, direction, properties)
...
class KeyMaps:
	...
	def new(self, space, idname, type, value, properties,
			...
			any=False, modal=False, repeat=False, direction=''):
		key_maps_new(
			self, space, idname, type, value, properties,
			alt, ctrl, shift, any, modal, repeat, direction
		)
	...
	def mute(self, space, idname, inputtype, value,
		...
		key_maps_mute(self, space, idname, inputtype, value,
			alt, ctrl, shift, any, modal, direction
		)
	
	def set_mute(self, state, delay):
		key_maps_set_mute(self, state)

	def register(self):
		key_maps_register(self)
	
	def unregister(self):
		key_maps_unregister(self)