...
def get_teapot_mesh(radius,csegs,body,handle,spout,lid):
	...
	def create_body(line, flip):
		...
		""" Create vertexes """
		path = get_path(line)
		sides = get_verts(path, csegs, radius)
		l = len(sides)-1
		...
		for i in range(cs):
			if flip:
				faces.append((f,f+i,f+i+1))
			else:
				faces.append((f+i+1,f+i,f))
		if flip:
			faces.append((f,f+cs,f+1))
		else:
	...
	def create_pipe(line1, line2, flip):
		f = len(verts)
		""" create vertexes """
		spline1 = get_path(line1)
		spline2 = get_path(line2)
		v1 = get_verts(spline1,csegs,radius)
		v2 = get_verts(spline2,csegs,radius)
		for p1,p2 in zip(v1,v2):
			ring = get_ring(p1,p2,1)
			vert = get_verts(ring,csegs,1)
			for v in vert:
				verts.append(v)
		""" faces """
		ssegs = csegs*2
	...
	if handle:
		create_pipe("handle_a", "handle_b", True)
	if spout:
		create_pipe("spout_a", "spout_b", False)
