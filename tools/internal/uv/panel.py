...
class UV_OP_Property_Panel(Panel):
	...
	def draw(self, ctx):
		...
		row = box.row(align=True)
		row.operator(
			'uv.snap_selected', text=" ", icon='SNAP_VERTEX'
		).target='PIXELS'

		row.operator(
			'uv.snap_selected', text=" ", icon='PIVOT_CURSOR'
		).target='CURSOR'

		row.operator(
			'uv.snap_selected', text=" ", icon='ORIENTATION_GIMBAL'
		).target='CURSOR_OFFSET'

		row.operator(
			'uv.snap_selected', text=" ", icon='SELECT_SET'
		).target='ADJACENT_UNSELECTED'

		row = box.row(align=True)
		row.operator(
			'uv.snap_cursor', text="Cursor", icon='ORIENTATION_CURSOR'
		).target='PIXELS'
		
		row.operator(
			'uv.snap_cursor', text="Vertex", icon='SNAP_VERTEX'
		).target='SELECTED'
		
		box.prop(
			ctx.space_data.uv_editor, 'lock_bounds',
		...
		row.operator(
			'uv.snap_selected', text=" To Cursor", icon='ORIENTATION_CURSOR'
		).target='CURSOR'

		row.operator(
			'uv.remove_doubles', text=" Merge", icon='DRIVER_DISTANCE'
		)
		...
		row = box.row(align=True)
		row.operator(
			'uv.mark_seam', text=" Mark Seam", icon='MESH_PLANE'
		).clear=False
		
		row.operator(
			'uv.mark_seam', text=" Clear Seam", icon='SELECT_SET'
		).clear=True
		...
		row.operator(
			'uv.cube_project', text=" Cube Projection", icon='MESH_CUBE'
		)

		row.operator(
			'uv.cylinder_project', text=" Cylinder Projection",
			icon='MESH_CYLINDER'
		)

		row.operator(
			'uv.sphere_project', text=" Sphere Projection",
			icon='MESH_UVSPHERE'
		)
		...
		row.operator(
			'uv.follow_active_quads', text=" Fallow Active Quad",
		...
		row.operator(
			'uv.align', text="Flatten", icon='NODE_CORNER'
		).axis='ALIGN_AUTO'

		row.operator(
			'uv.align', text="X", icon='NODE_SIDE'
		).axis='ALIGN_X'

		row.operator(
			'uv.align', text="Y", icon='NODE_TOP'
		).axis='ALIGN_Y'
