...
class VectorRange2:
	...
	def copy(self, full=False):
		vector = Vector2(self._x, self._y)
		if full:
			vector.lock = self.lock
			vector.auto = self.auto
			vector.limit = self.limit.copy()
			vector.min = self.min.copy()
			vector.max = self.max.copy()
			vector.default = self.default.copy()
		return(vector)
	...
	def x(self, x):
		if self.lock:
			...
			#TODO replace with a readable clear code
			self._x = self.min.x if x < self.min.x else self.max.x \
				if x > self.max.x else x
		else:
			self._x = x
	...
	def y(self, y):
		if self.lock:
			...
			#TODO replace with a readable clear code
			self._y = self.min.y if y < self.min.y else self.max.y \
				if y > self.max.y else y
		else:
			self._y = y 
...
class Range:
	...
	def value(self, value):
		self._value = self.min if value < self.min else self.max \
			if value > self.max else value
...
class Scale:
	...
	def copy(self):
		return Scale(
			self.enabled, self.left, self.right, self.top,
			self.bottom, self.sensitive
		)
