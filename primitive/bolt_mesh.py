...
def Build_Face_List_Quads(OFFSET, COLUMN, ROW, FLIP=0):
	...
	for j in range(ROW):
		for i in range(COLUMN):
			...
			if FLIP:
				Ret.append([
					OFFSET+Res1, OFFSET+Res2, OFFSET+Res3, OFFSET+Res4
				])
			else:
				Ret.append([
					OFFSET+Res4, OFFSET+Res3, OFFSET+Res2, OFFSET+Res1
				])
...
def Fill_Ring_Face(OFFSET, NUM, FACE_DOWN=0):
	...
	for i in range(NUM - 2):
		if (i % 2):
			...
			if FACE_DOWN:
				Ret.append([
					OFFSET+Face[2], OFFSET+Face[1], OFFSET+Face[0]
				])
			else:
				Ret.append([
	...
	for i in Lookup:
		if FLIP:
			if len(i) == 3:
				...
				faces.append([
					OFFSET + i[3], OFFSET + i[2], OFFSET + i[1], OFFSET + i[0]
				])
		else:
			if len(i) == 3:
				faces.append([OFFSET + i[0], OFFSET + i[1], OFFSET + i[2]])
...
def Create_Pan_Head(
		HOLE_DIA, HEAD_DIA, SHANK_DIA, HEIGHT,
		RAD1, RAD2, FACE_OFFSET, DIV_COUNT):
	...
	# for i in range(0,30,10):  was 0 to 30 more work
	...
	for i in range(20, 140, 10):
		...
		if ((0.0 - EndZOffset) + z) < (0.0 - HEIGHT):
			...
			verts.append([
				(HEAD_RADIUS - EndRad) + x, 0.0, (0.0 - EndZOffset) + z
			])
...
def Create_CounterSink_Head(
		HOLE_DIA, HEAD_DIA, SHANK_DIA, HEIGHT, RAD1, DIV_COUNT):
...
def Create_Cap_Head(
		HOLE_DIA, HEAD_DIA, SHANK_DIA, HEIGHT, RAD1, RAD2, DIV_COUNT):
...
#	 print (degrees (FLANGE_ANGLE_RAD))
	...
	v_Extended_Flange_Edge = Vector([
		0.0,
		0.0,
		-HEIGHT + FLANGE_HEIGHT + (tan(FLANGE_ANGLE_RAD)* FLANGE_RADIUS)
	])
#	 print("v_Extended_Flange_Edge")
#	 print (v_Extended_Flange_Edge)

	#0deg
	v_Flange_Edge = Vector([
		sin(radians(0)) * FLANGE_RADIUS,
		cos(radians(0)) * FLANGE_RADIUS,
		-HEIGHT + FLANGE_HEIGHT
	])
	v_Cross = geometry.intersect_line_line(
		v_0_Deg_Top_Point,
		Vector([
			v_0_Deg_Top_Point.x,
			v_0_Deg_Top_Point.y,-HEIGHT]),
	...
	#30deg
	v_Flange_Edge = Vector([
		sin(radians(30)) * FLANGE_RADIUS,
		cos(radians(30)) * FLANGE_RADIUS,
		-HEIGHT + FLANGE_HEIGHT
	])
	...
	verts.append([
		sin(radians(10)) * SHANK_RADIUS,
		cos(radians(10)) * SHANK_RADIUS,
		-HEIGHT
	])
	...
	verts.append([
		sin(radians(20)) * SHANK_RADIUS,
		cos(radians(20)) * SHANK_RADIUS,
		-HEIGHT
	])
	...
	verts.append([
		sin(radians(30)) * SHANK_RADIUS,
		cos(radians(30)) * SHANK_RADIUS,
		-HEIGHT
	])
...
def Thread_Start3(
		verts, INNER_RADIUS, OUTTER_RADIUS, PITCH, DIV_COUNT,
...
def Create_Thread_Verts(
		INNER_DIA, OUTTER_DIA, PITCH, HEIGHT,
	...
	Num = int(
		(HEIGHT - ((NUM_OF_START_THREADS * PITCH) +\
		(NUM_OF_END_THREADS * PITCH))) / PITCH
	)
	Row = 0
...
def Create_External_Thread(
		SHANK_DIA, SHANK_LENGTH, INNER_DIA, OUTTER_DIA,
	...
	Shank_Verts, Shank_Row, Offset = Create_Shank_Verts(
		SHANK_DIA, OUTTER_DIA, SHANK_LENGTH, Offset, DIV_COUNT
	)
...
def add_Nylon_Head(OUTSIDE_RADIUS, Z_LOCATION, DIV_COUNT):
	...
	for i in range(180, 80, -10):
		...
		z = cos(radians(i)) * RAD1
		verts.append([
			(OUTSIDE_RADIUS - RAD1) + x,
			0.0,
			((Height_Offset - OVER_ALL_HEIGHT) + RAD1) + z
		])
...
def Create_Internal_Thread_Start_Verts(
		verts, INNER_RADIUS, OUTTER_RADIUS, PITCH, DIV,
		CREST_PERCENT, ROOT_PERCENT, Height_Offset):
...
def Create_Internal_Thread_End_Verts(
		verts, INNER_RADIUS, OUTTER_RADIUS, PITCH,
		CREST_PERCENT, ROOT_PERCENT, Height_Offset, DIV_COUNT):
	...
	for j in range(2):
		...
		for i in range(DIV_COUNT + 1):
			...
			if j == Num:
				...
				x = sin(
					radians(i*Deg_Step))*(INNER_RADIUS+(i*Rank+rand_offset)
				)
...
def Create_Internal_Thread(
	...
	Row_Inc, Height_Offset = Create_Internal_Thread_Start_Verts(
		verts, INNER_RADIUS, OUTTER_RADIUS, PITCH,
		DIV_COUNT, CREST_PERCENT, ROOT_PERCENT, Height_Offset
	)
	...
	Row_Inc, Height_Offset = Create_Internal_Thread_End_Verts(
		...
		PITCH, CREST_PERCENT, ROOT_PERCENT, Height_Offset, DIV_COUNT
	)
...
def Nut_Mesh(props):
	...
	if props.bf_Nut_Type == 'bf_Nut_12Pnt':
		Head_Verts, Head_Faces, Lock_Nut_Rad = add_12_Point_Nut(
			...
			max(
				props.bf_12_Point_Nut_Flange_Dia,
				props.bf_12_Point_Nut_Flat_Distance
			)
		)
	else:
	...
	if props.bf_Nut_Type == 'bf_Nut_Lock':
		...
		Nylon_Head_Verts, Nylon_Head_faces, LowZ = add_Nylon_Head(
			Lock_Nut_Rad, 0 - New_Nut_Height, props.bf_Div_Count
		)
...
def Bolt_Mesh(props):
	...
	if props.bf_Bit_Type == 'bf_Bit_Allen' and props.bf_Head_Type == 'bf_Head_Pan':
		...
		mbd = Allen_Bit_Dia(props.bf_Allen_Bit_Flat_Distance)
		mpbd = Max_Pan_Bit_Dia(props.bf_Pan_Head_Dia)
		if mbd > mpbd:
	...
	if props.bf_Bit_Type == 'bf_Bit_Allen':
		Bit_Verts, Bit_Faces, Bit_Dia = Create_Allen_Bit(
			ReSized_Allen_Bit_Flat_Distance,props.bf_Allen_Bit_Depth
		)
	...
	if props.bf_Head_Type == 'bf_Head_Hex':
		Head_Verts, Head_Faces, Head_Height = Create_Hex_Head(
			props.bf_Hex_Head_Flat_Distance,
			Bit_Dia,
			props.bf_Shank_Dia,
			props.bf_Hex_Head_Height
		)
		...
		Head_Verts, Head_Faces, Head_Height = Create_12_Point_Head(
			...
			Bit_Dia,
			props.bf_Shank_Dia,
			props.bf_12_Point_Head_Height,
			#Limit the size of the Flange to avoid calculation error
			max(
				props.bf_12_Point_Head_Flange_Dia,
				props.bf_12_Point_Head_Flat_Distance
			)
		)
