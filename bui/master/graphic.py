...
from .classes import Colors, Vector2, Corner, Align
...
class Rectangle(Graphic):
	def __init__(self, owner, width=0, height=0, fillet=[0, 0, 0, 0]):
		...
		self.height = height
		self._height = height
		up_left, up_right, down_left, down_right = fillet
		self.fillet = Corner(up_left, up_right, down_left, down_right)
		self._fillet = Corner(up_left, up_right, down_left, down_right)
		self.create_shape()
	...
	def create_shape(self):
		...
		pos = self.owner.location + self.offset + \
			self.align.location(self.owner.size, size)
		
		verts, inds = [], []
		center = Vector2(pos.x + width / 2, pos.y + height / 2)
		verts.append((center.x, center.y))
		
		verts += self.get_corner(
			center, width/2, height/2, self.fillet.top_right, 1, 1
		)

		verts += self.get_corner(
			center, width/2, height/2, self.fillet.bottom_right, 1, -1
		)

		verts += self.get_corner(
			center, width/2, height/2, self.fillet.bottom_left, -1, -1
		)

		verts += self.get_corner(
			center, width/2, height/2, self.fillet.top_left, -1, 1
		)
