...
def get_modifiers_list(scene, ctx):
	modifiers = [mode.type.capitalize() for mode in ctx.active_object.modifiers]
	return [(m,m,'') for m in modifiers]

class modifiers_list(PropertyGroup):
	modifiers: EnumProperty(name="Modifier",items = get_modifiers_list)

def get_selected_modifier(obj,modifier):
	for m in obj.modifiers:
		if m.type == modifier.upper():
			return m

""" 
*	This functions are copyed from ''class DATA_PT_modifiers(ModifierButtonsPanel, Panel)''
*	whit out any changes for update just copy them all.
"""
...
""" ^^^^^ end of copy past part here ^^^^^ """

class Modifier_OT_MultiEdit(Operator):
	bl_idname = "modifier.edit_multi"
	bl_label = "Edit Multi Modifiers"
	bl_description = "Match modifier with selection if have same modifier"

	def draw(self, ctx):
		modifier = get_selected_modifier(ctx.active_object, ctx.scene.active_modifiers.modifiers)

		col = self.layout.column()
		box = col.box()
		# box.operator_menu_enum("object.modifier_add", "type")
		box.prop(ctx.scene.active_modifiers, "modifiers", text="Modifiers")
		...
		box = col.box()
		exec(modifier.type + "(self, box, ctx.active_object, modifier)")
		
		box = col.box()
		box.operator("modifier.match_selection",icon='UV_SYNC_SELECT').name = modifier.name
...
def register_edit():
	bpy.utils.register_class(modifiers_list)
	bpy.types.Scene.active_modifiers = PointerProperty(type=modifiers_list)
